...
import UserTableRow from '../../src/pages/ProductReviewRow';
...
const ProductReview = () => {
  const {ProductReviewData,setProductReviewData,ProductReviewsetEditData}= useProductReview()
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = ProductReviewData?.map((n) => n.id);
  ...
  const handleDelete = (id) => {
    const updatedData = ProductReviewData?.filter((item) => item.id !== id);
    const updatedDataWithSequentialIds = updatedData?.map((item, index) => ({
      ...item,
      id: index + 1,
    }));
    setProductReviewData(updatedDataWithSequentialIds);
  };

  

  const dataFiltered = applyFilter({
    inputData: ProductReviewData,
  ...
  return (
    <Container>
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
        <Typography variant="h4">Product Review</Typography>

        <Button
          variant="contained"
          color="inherit"
          startIcon={<Iconify icon="eva:plus-fill" />}
          onClick={() => navigate('/addProductReview')}
        >
          Add Record
        </Button>
      </Stack>
          ...
          <TableContainer sx={{ overflow: 'unset' }}>
            <Table sx={{ minWidth: 800 }}>
              ...
              <TableBody>
                ...
                <TableEmptyRows
                  height={77}
                  emptyRows={emptyRows(page, rowsPerPage, ProductReviewData?.length)} 
                />
