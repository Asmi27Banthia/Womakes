...
import UserTableRow from '../../src/pages/PaymentRow';
...
const Payment = () => {
  const {PaymentData,setPaymentData,PaymentsetEditData}= usePayment()
  ...
  const handleEditOpen = (row) => {
    console.log('row', row)
    console.log('PaymentsetEditData', PaymentsetEditData)
    PaymentsetEditData(row);
    navigate("/addPayment")
  };
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = PaymentData?.map((n) => n.id);
  ...
  const handleDelete = (id) => {
    ...
    setPaymentData(updatedDataWithSequentialIds);
  };
            ...
            <Table sx={{ minWidth: 800 }}>
              <UserTableHead
                ...
                headLabel={[
                  ...
                  { id: 'paymentReference_Id', label: 'PaymentReferenceId' },
