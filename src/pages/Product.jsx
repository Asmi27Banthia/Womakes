...
import UserTableRow from '../../src/pages/ProductRow';
...
import AddRecords from 'src/forms/AddProduct';
import { useNavigate } from 'react-router-dom';
import { useProduct } from 'src/hooks/useProduct';


const Product = () => {
  const {ProductData,setProductData,ProductsetEditData}= useProduct()
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = ProductData?.map((n) => n?.Product_name);
  ...
  const handleDelete = (id) => {
    ...
    setProductData(updatedDataWithSequentialIds);
  };
            ...
            <Table sx={{ minWidth: 800 }}>
              <UserTableHead
                ...
                headLabel={[
                  ...
                  { id: 'approvedDate', label: 'ApprovedDate' },
                  { id: 'Vendor_Id', label: 'VendorId' },
              ...
              <TableBody>
                {dataFiltered.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)?.map((row) => (
                  <UserTableRow
                    key={row.id}
                    {...row}
                    selected={selected.indexOf(row?.Product_name) !== -1}
                    handleClick={(event) => handleClick(event, row?.Product_name)}
