...
import UserTableRow from '../../src/pages/WareHouseRow';
...
const WareHouse = () => {
  const {WarehouseData,setWarehouseData,WarehousesetEditData}= useWarehouse()
  ...
  const handleEditOpen = (row) => {
    console.log('row', row)
    console.log('WarehousesetEditData', WarehousesetEditData)
    WarehousesetEditData(row);
  ...
  const handleDelete = (id) => {
    const updatedData = WarehouseData?.filter((item) => item.id !== id);
    const updatedDataWithSequentialIds = updatedData?.map((item, index) => ({
      ...item,
      id: index + 1,
    }));
    setWarehouseData(updatedDataWithSequentialIds);
  };
          ...
          <TableContainer sx={{ overflow: 'unset' }}>
            <Table sx={{ minWidth: 800 }}>
              ...
              <TableBody>
                ...
                <TableEmptyRows
                  height={77}
                  emptyRows={emptyRows(page, rowsPerPage, WarehouseData?.length)} 
                />
