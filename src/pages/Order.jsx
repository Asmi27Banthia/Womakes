...
import UserTableRow from '../../src/pages/OrderRow';
...
const Order = () => {
  const {OrderData,setOrderData,OrdersetEditData}= useOrder()
  ...
  const handleDelete = (id) => {
    const updatedData = OrderData?.filter((item) => item.id !== id);
    const updatedDataWithSequentialIds = updatedData?.map((item, index) => ({
      ...item,
      id: index + 1,
    }));
    setOrderData(updatedDataWithSequentialIds);
  };
            ...
            <Table sx={{ minWidth: 800 }}>
              <UserTableHead
                ...
                headLabel={[
                  ...
                  { id: 'cart_Id', label: 'CartId' },
                  { id: 'baseTotal', label: 'BaseTotal(₹)'},
                  { id: 'grandTotal', label: 'GrandTotal(₹)' },
                  { id: 'discountAmount', label: 'DiscountAmount(₹)' },
                  { id: 'discountCode', label: 'DiscountCode' },
                  { id: 'status', label: 'Status' },
                  { id: 'address_Id', label: 'AddressId' },
                  { id: 'paymentStatus', label: 'PaymentStatus' },
                  { id: 'deliveryCharge', label: 'DeliveryCharge(₹)'},
                  { id: '', label: '' },
                ]}
              />
              <TableBody>
